...
const CodeChefLogo = ({ size = 8, className = "" }) => {
  const baseHeight = 40; 
  ...
  return (
    <div className={`flex items-center ${className}`}>
      <svg 
        ...
        style={{ color: '#87CEFA' }} // Light ice blue for brackets
      >
        <path d={leftBracketPath} />
      </svg>
      <span
        className="font-extrabold mx-1 select-none"
        style={{ fontSize: `${textFontSize}px`, lineHeight: '1', color: '#00BFFF' }} // Bright ice blue text
      >
        CC
      </span>
      <svg 
